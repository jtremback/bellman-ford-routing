...
function feasibilityCondition (self, newSource, existingSource) {
...
function receiveUpdate (self, from, newSources) {
  ...
  for (let newSource of newSources) {
    ...
    if (feasibilityCondition(self, newSource, existingSource)) {
      ...
      ui.log(`${self.id} accepted update ${infinityJSON.stringify(newSource)} from ${from.id}, with feasibility distance ${existingSource.fd}`)
      ui.updateNetwork(network)
